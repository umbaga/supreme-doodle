        ...
        pool.connect(function(err, client, done) {
            ...
            async.waterfall([
                function init(cb) {
                    ...
                    if (resObj._template.description && resObj._template.description.length != 0) {
                ...
                function checkExistingStuff(resObj, callback) {
                    ...
                    query.on('end', function() {
                        if (results.length != 0) {
                            ...
                            resObj._template.results[0].descriptionId = results[0].descriptionId;
                        }
                        return callback(null, resObj);
                    });
                },
                        ...
                        query.on('end', function() {
                            for (let q = 0; q < results.length; q++) {
                                if (results[q].typeId == itemtypes.TYPE.DESCRIPTION.GENERAL) {
                                    resObj._template.descriptionId = results[q].id;
                                }
                            }
                            return callback(null, resObj);
                        });
                        ...
                        query.on('end', function() {
                            for (let q = 0; q < results.length; q++) {
                                ...
                                if (results[q].typeId == itemtypes.TYPE.DESCRIPTION.SUGGESTED_CHARACTERISTICS) {
                                    resObj._template.suggestedCharacteristicsId = results[q].id;
                                }
                            }
                            return callback(null, resObj);
                        });
